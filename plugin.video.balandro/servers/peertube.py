...
def get_video_url(page_url, url_referer=''):
    ...
    try:
       if "|Referer=" in page_url:
           page_url = page_url.split("|Referer=")[1]

           if '/video.doramed.tv/' in page_url: ini_page_url = page_url
    except:
       pass

    if '/download/videos/' in page_url or '/download/web-videos/' in page_url:
    ...
    if not '/video.doramed.tv/' in page_url:
    ...
    if not video_urls:
        if xbmc.getCondVisibility('System.HasAddon("script.module.resolveurl")'):
            path = translatePath(os.path.join('special://home/addons/script.module.resolveurl/lib/resolveurl/plugins/', 'peertube.py'))
            ...
            if config.get_setting('servers_time', default=True):
                platformtools.dialog_notification('Cargando [COLOR cyan][B]Peertube[/B][/COLOR]', 'Espera requerida de %s segundos' % espera)
