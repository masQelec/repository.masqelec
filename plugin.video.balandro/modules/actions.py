# -*- coding: utf-8 -*-
# ------------------------------------------------------------
# WebTV - Ejecución de acciones concretas que no devuelven ningún listado
# ------------------------------------------------------------
...
def show_help_use(item):
    ...
    platformtools.dialog_textviewer('Ejemplos de uso de WebTV', txt)


def show_help_faq(item):
    logger.info()

    txt = '[COLOR gold]¿ De dónde viene WebTV ?[/COLOR][CR]'
    txt += 'WebTV es un addon derivado de Pelisalacarta y Alfa, simplificado a nivel interno de código y a nivel de uso por parte del usuario.'
    ...
    txt += ' Al ser un addon de tipo navegador, tiene el nombre de un velero ya que el webtv era una embarcación ligera y maniobrable, muy apreciada por los piratas.'

    txt += '[CR][CR][COLOR gold]¿ Qué características tiene WebTV ?[/COLOR][CR]'
...
def show_help_tracking(item):
    ...
    txt += ' Estas marcas están en la base de datos de Kodi pero no en las propias de WebTV.'
