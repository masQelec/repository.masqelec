...
def do_downloadpage(url, post=None, headers=None):
    ...
    ant_hosts = ['https://pelisflix.beer/']
    ...
    hay_proxies = False
    if config.get_setting('channel_pelisflix_proxies', default=''): hay_proxies = True
    ...
    if not url.startswith(host):
        ...
        if not data:
            if not '/?s=' in url:
                if config.get_setting('channels_re_charges', default=True): platformtools.dialog_notification('PelisFlix', '[COLOR cyan]Re-Intentando acceso[/COLOR]')
...
def generos(item):
    ...
    matches = scrapertools.find_multiple_matches(bloque, '<a href="(.*?)".*?</svg>(.*?)</a>')
...
def list_all(item):
    ...
    matches = scrapertools.find_multiple_matches(data, '<div class="movie-item">(.*?)</div> </a>')
    ...
    if itemlist:
        if '<ul class="pagination">' in data:
            next_page = scrapertools.find_single_match(data, '<ul class="pagination">.*?<a class="disabled">.*?href="(.*?)"')
...
def temporadas(item):
    ...
    bloque = scrapertools.find_single_match(data, '<a class="btn"(.*?)</article>')

    seasons = scrapertools.find_multiple_matches(bloque, '<a href="(.*?)".*?</svg>.*?<span itemprop="name">Temporada(.*?)</span>')
...
def episodios(item):
    ...
    bloque = scrapertools.find_single_match(data, ">Temporadas y Capítulos<(.*?)>Sugerencias<")

    episodes = scrapertools.find_multiple_matches(bloque, '<meta itemprop="episodeNumber"(.*?)</span> </a>')
        ...
        if config.get_setting('channels_charges', default=True):
            ...
            if sum_parts >= 1000:
                ...
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]125[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('PelisFlix', '[COLOR cyan]Cargando 125 elementos[/COLOR]')
                    item.perpage = 125

            elif sum_parts >= 125:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]75[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('PelisFlix', '[COLOR cyan]Cargando 75 elementos[/COLOR]')
                    item.perpage = 75
    ...
    for match in episodes[item.page * item.perpage:]:
        ...
        epis = scrapertools.find_single_match(match, '<span itemprop="name">.*?<span>.*?x(.*?)$')
        epis = epis.replace('Episodio', '').strip()

        if not epis: epis = 1

        title = scrapertools.find_single_match(match, '<span itemprop="name">(.*?)</span>')

        title = title.replace('Ver ', '').strip().capitalize()
...
def findvideos(item):
    ...
    IDIOMAS = {'Latino': 'Lat', 'Español Castellano': 'Esp', 'Subtitulado': 'Vose'}

    data = do_downloadpage(item.url)
    data = re.sub(r'\n|\r|\t|\s{2}|&nbsp;', '', data)

    # embeds
    matches = scrapertools.find_multiple_matches(data, '<li role="presentation"(.*?)</li>')
    ...
    for match in matches:
        ...
        data_url = scrapertools.find_single_match(match, 'data-server="(.*?)"')

        if not data_url: continue

        lang = scrapertools.find_single_match(match, '<span> Opción.*? · (.*?)</span>').strip()

        language = IDIOMAS.get(lang, lang)

        itemlist.append(Item( channel = item.channel, action = 'play', server = '', durl = data_url, language = language ))
