...
def list_all(item):
    ...
    if item.group == 'cupo':
        bloque = scrapertools.find_single_match(data, '</i> Películas online</h2>(.*?)>© Pelisflix3')
    else:
        bloque = scrapertools.find_single_match(data, '<h1(.*?)>© Pelisflix3')
        if not bloque: bloque = scrapertools.find_single_match(data, 'Series nuevas(.*?)>© Pelisflix3')
    ...
    if item.page == 0:
        ...
        if sum_parts > 250:
            if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]250[/B][/COLOR] elementos?'):
                platformtools.dialog_notification('PelisFlix3', '[COLOR cyan]Cargando elementos[/COLOR]')
                item.perpage = 250
