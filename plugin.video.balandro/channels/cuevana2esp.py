    ...
    if not url.startswith(host) and not _player in url:
        ...
        if not data:
            if not 'search?q=' in url:
                if config.get_setting('channels_re_charges', default=True): platformtools.dialog_notification('Cuevana2Esp', '[COLOR cyan]Re-Intentando acceso[/COLOR]')
...
def episodios(item):
    ...
    if item.page == 0 and item.perpage == 50:
        ...
        if config.get_setting('channels_charges', default=True):
            ...
            if sum_parts >= 100:
                platformtools.dialog_notification('Cuevana2Esp', '[COLOR cyan]Cargando Temp. ' + str(item.contentSeason) + ' sobre ' + str(sum_parts) + ' elementos[/COLOR]')
        elif tvdb_id:
            if sum_parts > 50:
...
def _news(item):
    ...
    item.url = host + 'archives/movies/releases'
...
def _lasts(item):
    ...
    item.url = host + 'archives/series/releases'
...
def _epis(item):
    ...
    item.url = host + 'archives/episodes'
    item.search_type = 'tvshow'
