...
def do_downloadpage(url, post=None, headers=None, raise_weberror=True):
    ...
    if not url.startswith(host):
        ...
        if not data:
            if not '?query=' in url and not 'buscar?q=' in url:
                if config.get_setting('channels_re_charges', default=True): platformtools.dialog_notification('PediaATorrent', '[COLOR cyan]Re-Intentando acceso[/COLOR]')
    ...
    if not '?query=' in url and not 'buscar?q=' in url:
        ...
        elif '<title>Just a moment...</title>' in data:
