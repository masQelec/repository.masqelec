...
def do_downloadpage(url, post=None, headers=None):
    ...
    if 'Checkpoint</title>' in data:
        if not '#results' in url:
            if config.get_setting('channels_re_charges', default=True): platformtools.dialog_notification('TGx', '[COLOR cyan]Re-Intentanto acceso[/COLOR]')
            ...
            if 'Checkpoint</title>' in data:
                platformtools.dialog_notification(config.__addon_name, '[COLOR red][B]CloudFlare[COLOR orangered] Protection [COLOR yellowgreen]CheckPoint[/B][/COLOR]')
                return ''
...
def mainlist_pelis(item):
    ...
    itemlist.append(item.clone( title = 'Catálogo', action = 'list_all', url = host + 'torrents.php', search_type = 'movie' ))

    itemlist.append(item.clone( title = 'En castellano', action = 'list_all', url = host + 'torrents.php?search=&lang=6&nox=2&page=0', search_type = 'movie', text_color='moccasin' ))

    itemlist.append(item.clone( title = 'Colecciones', action = 'list_all', url = host + 'torrents.php?search=&lang=0&nox=2&page=0', search_type = 'movie', text_color='greenyellow' ))
    ...
    itemlist.append(item.clone( title = 'En SD', action = 'list_all', url = host + 'torrents.php?c1=1&search=&lang=0&nox=2&page=0', search_type = 'movie' ))

    itemlist.append(item.clone( title = 'En TS', action = 'list_all', url = host + 'torrents.php?search=&cat=45&page=0', search_type = 'movie' ))
...
def mainlist_series(item):
    ...
    itemlist.append(item.clone( title = 'Catálogo', action = 'list_all', url = host + 'torrents.php', search_type = 'tvshow' ))

    itemlist.append(item.clone( title = 'En castellano', action = 'list_all', url = host + 'torrents.php?search=&lang=6&nox=2&page=0', search_type = 'tvshow', text_color='moccasin' ))
    ...
    itemlist.append(item.clone( title = ' - En SD', action = 'list_all', url = host + 'torrents.php?c5=1&search=&lang=0&nox=2&page=0', search_type = 'tvshow' ))

    itemlist.append(item.clone( action='generos', title='Por género', search_type = 'tvshow' ))
    itemlist.append(item.clone( action='idiomas', title = 'Por idioma', search_type = 'tvshow' ))
...
def mainlist_adults(item):
    ...
    itemlist.append(item.clone( title = ' - En SD', action = 'list_all', url = host + 'torrents.php?c34=1&search=&lang=0&nox=2&page=0', search_type = 'movie' ))
...
def generos(item):
    ...
    data = do_downloadpage(host + 'torrents.php')
    data = re.sub(r'\n|\r|\t|\s{2}|&nbsp;', '', data)

    bloque = scrapertools.find_single_match(data, '>Genre search<(.*?)</table>')

    matches = re.compile("value='(.*?)'.*?<a href='(.*?)'.*?>(.*?)</a>", re.DOTALL).findall(bloque)
...
def idiomas(item):
    ...
    bloque = scrapertools.find_single_match(data, '<option value="0">All languages</option>(.*?)</select>')
    ...
    for value, title in matches:
        ...
        if 'Other' in title: title = 'Z ' + title

        url = host + 'torrents.php?c4=1&search=&lang=' + value + '&nox=2'
...
def list_all(item):
    ...
    if not no_porns:
        if '?search=' in item.url: item_url = item.url + '&nox=1'
        else: item.url = item.url + '?nox=1'

    data = do_downloadpage(item.url)
    data = re.sub(r'\n|\r|\t|\s{2}|&nbsp;', '', data)

    matches = scrapertools.find_multiple_matches(data, "<div class='tgxtablecell shrink rounded txlight'>(.*?)onmouseout='return nd")
    if not matches: matches = scrapertools.find_multiple_matches(data, "<div class='tgxtablecell shrink rounded txlight'>(.*?)</table>")
    ...
    if itemlist:
        next_page = scrapertools.find_single_match(data, "</table></div></div></div></div><center><nav aria-label='pages'><ul class='pagination.*?current.*?href='(.*?)'")
...
def findvideos(item):
    ...
    bloque = scrapertools.find_single_match(data, '>Download<(.*?)</tr><tr><td>')
    ...
    for url in matches:
        ...
        elif not '/watercache.' in url: continue
...
def search(item, texto):
    logger.info()
    try:
        item.url = host + 'torrents.php?search=' + texto.replace(" ", "+") + '&lang=0&nox=2#results'
