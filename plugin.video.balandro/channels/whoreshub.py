...
def list_all(item):
    ...
    bloque = scrapertools.find_single_match(data, '<div class="thumbs" id="list(.*?)</script>')

    patron = '<a class="item".*?href="(.*?)".*?title="(.*?)".*?data-src="(.*?)"(.*?)</span></span></span></li>'

    matches = scrapertools.find_multiple_matches(bloque, patron)

    for url, title, thumb, resto in matches:
        duration = scrapertools.find_single_match(resto, '<span class="duration">(.*?)</span>').strip()
    ...
    if itemlist:
        bloque = scrapertools.find_single_match(data, '<ul class="pagination"(.*?)</ul>')

        next_page = scrapertools.find_single_match(bloque, '<li class="current">.*?</li>.*?<a href="(.*?)"')
...
def listas(item):
    ...
    bloque = scrapertools.find_single_match(data, '<div class="thumbs" id="list(.*?)</html>')
    if not bloque: bloque = scrapertools.find_single_match(data, 'id="list_models_models_list_items">(.*?)</html>')
    if not bloque: bloque = scrapertools.find_single_match(data, '<div class="main-container">(.*?)</html>')

    patron = '<a class="item".*?href="([^"]+)".*?title="([^"]+)".*?src="([^"]+)"'

    if '/models/' in item.url: patron = '<a class="item".*?href="([^"]+)".*?title="([^"]+)".*?data-src="([^"]+)"'

    elif '/sites/' in item.url: patron = '<a href="(.*?)".*?<span class="name">(.*?)</span>.*?</a>(.*?)</li>'
    ...
    if '/sites/' in item.url: return sorted(itemlist,key=lambda x: x.title)
