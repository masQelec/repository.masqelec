...
def item_configurar_proxies(item):
    ...
    if config.get_setting('channel_documaniatv_proxies', default=''):
...
def do_downloadpage(url, headers=None, post=None):
    ...
    hay_proxies = False
    if config.get_setting('channel_documaniatv_proxies', default=''): hay_proxies = True
    ...
    if not url.startswith(host):
        ...
        if hay_proxies:
            data = httptools.downloadpage_proxy('documaniatv', url, post=post, headers=headers, timeout=timeout).data
        else:
            data = httptools.downloadpage(url, post=post, headers=headers, timeout=timeout).data

        if not data:
            if not 'search.php?keywords=' in url:
                if config.get_setting('channels_re_charges', default=True): platformtools.dialog_notification('DocumaniaTv', '[COLOR cyan]Re-Intentanto acceso[/COLOR]')
...
def acciones(item):
    ...
    itemlist.append(item.clone( channel='submnuctext', action='_test_webs', title='Test Web del canal [COLOR yellow][B] ' + host + '[/B][/COLOR]',
                                from_channel='documaniatv', folder=False, text_color='chartreuse' ))
