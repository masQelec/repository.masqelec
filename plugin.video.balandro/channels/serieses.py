...
host = 'https://www.seriesespagnol.cc/'
...
def mainlist_series(item):
    ...
    itemlist.append(item.clone( title = 'Catálogo', action = 'list_all', url = host + 'ver-series-gratis.html', search_type = 'tvshow' ))
...
def anios(item):
    ...
    for x in range(current_year, 1954, -1):
        url = host + 'ver-series-gratis/ano/' + str(x) + '.html'
...
def alfabetico(item):
    ...
    for letra in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789':
        url = host + 'ver-series-gratis/alfabeto/' + letra
...
def list_all(item):
    ...
    bloque = scrapertools.find_single_match(data, '<span>Seriesespagnol.cc</span>(.*?)>mas vistas<')

    if not bloque: bloque = scrapertools.find_single_match(data, '<span>seriesespagnol.cc</span>(.*?)>mas vistas<')

    if not bloque: bloque = scrapertools.find_single_match(data, '<span>Seriesespagnol</span>(.*?)>mas vistas<')
    if not bloque: bloque = scrapertools.find_single_match(data, '<span>seriesespagnol</span>(.*?)>mas vistas<')
    ...
    for match in matches:
        ...
        year = '-'
        if '/ano/' in item.url:
            year = scrapertools.find_single_match(item.url, "/ano/(.*?).html")
            if year:
...
def episodios(item):
    ...
    if item.page == 0 and item.perpage == 50:
        ...
        if config.get_setting('channels_charges', default=True):
            ...
            if sum_parts >= 100:
                platformtools.dialog_notification('SeriesEs', '[COLOR cyan]Cargando ' + str(sum_parts) + ' elementos[/COLOR]')
        elif tvdb_id:
            if sum_parts > 50:
                platformtools.dialog_notification('SeriesEs', '[COLOR cyan]Cargando Todos los elementos[/COLOR]')
                item.perpage = sum_parts
        else:
            item.perpage = sum_parts
            ...
            if sum_parts >= 1000:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]500[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('SeriesEs', '[COLOR cyan]Cargando 500 elementos[/COLOR]')
                    item.perpage = 500

            elif sum_parts >= 500:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]250[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('SeriesEs', '[COLOR cyan]Cargando 250 elementos[/COLOR]')
                    item.perpage = 250

            elif sum_parts >= 250:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]125[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('SeriesEs', '[COLOR cyan]Cargando 125 elementos[/COLOR]')
                    item.perpage = 125

            elif sum_parts >= 125:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]75[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('SeriesEs', '[COLOR cyan]Cargando 75 elementos[/COLOR]')
                    item.perpage = 75
