...
def list_all(item):
    ...
    if '<h2>Añadido recientemente' in data:
        bloque = scrapertools.find_single_match(data, '<h2>Añadido recientemente(.*?)© 2022 por Pelispluss')
    elif '<h1>Tendencias' in data:
        bloque = scrapertools.find_single_match(data, '<h1>Tendencias(.*?)© 2022 por Pelispluss')
    elif '<h1>Ratings' in data:
        bloque = scrapertools.find_single_match(data, '<h1>Ratings(.*?)© 2022 por Pelispluss')
    else:
        bloque = data
    ...
    for match in matches:
        ...
        if not '/ver-pelicula/' in url: continue
...
def findvideos(item):
    ...
    for match in matches:
        ...
        elif 'mystream' in servidor: continue
...
def list_search(item):
    ...
    bloque = scrapertools.find_single_match(data, '<h1>Resultados encontrados(.*?)© 2022 por Pelispluss')
