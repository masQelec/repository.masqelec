...
def list_all(item):
    ...
    bloque = scrapertools.find_single_match(data, '<div class="list-videos">(.*?)<div class="copyright">')

    patron = '<div class="item">.*?href="(.*?)".*?title="(.*?)".*?data-original="(.*?)"(.*?)</div></div></a></div>'
...
def listas(item):
    ...
    bloque = scrapertools.find_single_match(data, '<div class="list-videos">(.*?)</html>')
    ...
    if '/models/' in item.url: patron = '<a class="item".*?href="([^"]+)".*?title="([^"]+)".*?src="([^"]+)"'
    ...
    elif '/categories/' in item.url: return sorted(itemlist,key=lambda x: x.title)
