...
ant_hosts = ['https://www20.dontorrent.link/']
...
def item_configurar_proxies(item):
    ...
    if config.get_setting('channel_dontorrent21_proxies', default=''):
...
def do_downloadpage(url, post=None, headers=None):
    ...
    if not url.startswith(host):
        ...
        if hay_proxies:
            data = httptools.downloadpage_proxy('dontorrent21', url, post=post, headers=headers, timeout=timeout).data
        else:
            data = httptools.downloadpage(url, post=post, headers=headers, timeout=timeout).data

        if not data:
            if not '/buscar/' in url:
                if config.get_setting('channels_re_charges', default=True): platformtools.dialog_notification('DonTorrent21', '[COLOR cyan]Re-Intentando acceso[/COLOR]')
...
def acciones(item):
    ...
    itemlist.append(item.clone( channel='submnuctext', action='_test_webs', title='Test Web del canal [COLOR yellow][B] ' + host + '[/B][/COLOR]',
                                from_channel='dontorrent21', folder=False, text_color='chartreuse' ))
    ...
    itemlist.append(item.clone( channel='actions', action='show_old_domains', title='[COLOR coral][B]Historial Dominios[/B][/COLOR]', channel_id = 'dontorrent21' ))
    ...
    if item.server == 'torrent':
        ...
        if item.server == 'directo':
            ...
            if url_base64.startswith('magnet:'):
               ...
               if PY3:
                   from core import requeststools
                   data = requeststools.read(url_base64, '')
               else:
