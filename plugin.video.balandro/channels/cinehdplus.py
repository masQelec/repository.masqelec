...
def item_configurar_proxies(item):
    ...
    if config.get_setting('channel_cinehdplus_proxies', default=''):
...
def do_downloadpage(url, post=None, headers=None, raise_weberror=True):
    ...
    hay_proxies = False
    if config.get_setting('channel_cinehdplus_proxies', default=''): hay_proxies = True
    ...
    if not url.startswith(host):
        ...
        if hay_proxies:
            data = httptools.downloadpage_proxy('cinehdplus', url, post=post, headers=headers, raise_weberror=raise_weberror, timeout=timeout).data
        else:
            data = httptools.downloadpage(url, post=post, headers=headers, raise_weberror=raise_weberror, timeout=timeout).data

        if not data:
            if not '?story=' in url:
                if config.get_setting('channels_re_charges', default=True): platformtools.dialog_notification('CineHdPlus', '[COLOR cyan]Re-Intentanto acceso[/COLOR]')
...
def acciones(item):
    ...
    itemlist.append(item.clone( channel='submnuctext', action='_test_webs', title='Test Web del canal [COLOR yellow][B] ' + host + '[/B][/COLOR]',
                                from_channel='cinehdplus', folder=False, text_color='chartreuse' ))
...
def calidades(item):
    ...
    for value in matches:
        ...
        url = host + 'peliculas/?quality=' + value
...
def list_all(item):
    ...
    matches = re.compile('<div class="card ">(.*?)</div></div></div>').findall(data)
    ...
    for article in matches:
        ...
        langs = []
        if '/images/castellano' in article: langs.append('Esp')
        if '/images/latino' in article: langs.append('Lat')
        if not '/images/' in article: langs.append('Vose')

        year = scrapertools.find_single_match(article, '<span class="year">(.*?)</span>')
        if not year: year = '-'

        tipo = 'tvshow' if re.match(r"s\d+-e\d+", qlty) else 'movie'
    ...
    if itemlist:
        if '<div class="catalog__paginator">' in data:
            next_page = scrapertools.find_single_match(data, '<div class="catalog__paginator">.*?</span>.*?<a href="(.*?)"')
...
def episodios(item):
    ...
    if item.page == 0:
        ...
        if config.get_setting('channels_charges', default=True):
            ...
            if sum_parts >= 100:
                platformtools.dialog_notification('CineHdPlus', '[COLOR cyan]Cargando ' + str(sum_parts) + ' elementos[/COLOR]')
        elif tvdb_id:
            if sum_parts > 50:
                platformtools.dialog_notification('CineHdPlus', '[COLOR cyan]Cargando Todos los elementos[/COLOR]')
                item.perpage = sum_parts
        else:
            item.perpage = sum_parts
            ...
            if sum_parts >= 1000:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]500[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('CineHdPlus', '[COLOR cyan]Cargando 500 elementos[/COLOR]')
                    item.perpage = 500

            elif sum_parts >= 500:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]250[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('CineHdPlus', '[COLOR cyan]Cargando 250 elementos[/COLOR]')
                    item.perpage = 250

            elif sum_parts >= 250:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]100[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('CineHdPlus', '[COLOR cyan]Cargando 100 elementos[/COLOR]')
                    item.perpage = 100
