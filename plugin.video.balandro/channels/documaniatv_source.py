...
# Versiones Chrome
cod_cver = [70, 71, 78, 79, 80, 81, 83, 84, 85, 86, 87, 88]
...
ff_versions = [78, 79, 80, 81, 82, 83, 84]  # (Windows NT 10.0; Win64; x64; rv:%s.0)
...
def localize_header(test, last_chrome):
    ...
    for ilh in range(0, elements):
        ...
        try:
            ...
            resp = httptools.downloadpage_proxy(canal, url = url_check_header, headers = headers_check, timeout = 30)
            data = resp.data
            # logger.debug(data)
...
def do_downloadpage(url, headers = None):
    data = httptools.downloadpage_proxy(canal, url = url, headers = headers, timeout = 30).data
    return data
