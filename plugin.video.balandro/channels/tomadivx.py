...
def mainlist_pelis(item):
    ...
    itemlist.append(item.clone( title = 'En [COLOR moccasin]4K[/COLOR]', action = 'list_all', url = host + 'peliculas/4K/page/1', search_type = 'movie' ))
...
def list_all(item):
    ...
    if item.search_type == "movie":
        ...
        for url, title in matches:
            ...
            itemlist.append(item.clone( action = 'findvideos', url = host[:-1] + url, title = title, contentType = 'movie', contentTitle = titulo, contentExtra = 'documentary', infoLabels={'year': "-"} ))
