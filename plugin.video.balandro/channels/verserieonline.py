...
host = 'https://verserie.cc/'


def do_downloadpage(url, post=None, headers=None):
    if '/series-latino/año/' in url: raise_weberror = False
...
def mainlist_series(item):
    ...
    itemlist.append(item.clone( title = 'Catálogo', action = 'list_all', url = host + 'series-latino.html', search_type = 'tvshow' ))
...
def paises(item):
    ...
    itemlist.append(item.clone( title = 'América', action = 'list_all', url = host + 'series-latino/pais/usa.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'Argentina', action = 'list_all', url = host + 'series-latino/pais/ar.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'Brasil', action = 'list_all', url = host + 'series-latino/pais/br.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'Colombia', action = 'list_all', url = host + 'series-latino/pais/co.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'Corea', action = 'list_all', url = host + 'series-latino/pais/kr.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'Egipto', action = 'list_all', url = host + 'series-latino/pais/eg.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'España', action = 'list_all', url = host + 'series-latino/pais/es.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'Francia', action = 'list_all', url = host + 'series-latino/pais/fr.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'Italia', action = 'list_all', url = host + 'series-latino/pais/it.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'Japón', action = 'list_all', url = host + 'series-latino/pais/jp.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'México', action = 'list_all', url = host + 'series-latino/pais/mx.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'Reino Unido', action = 'list_all', url = host + 'series-latino/pais/gb.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'Suecia', action = 'list_all', url = host + 'series-latino/pais/se.html', text_color='hotpink' ))
    itemlist.append(item.clone( title = 'Turquía', action = 'list_all', url = host + 'series-latino/pais/tr.html', text_color='hotpink' ))
...
def anios(item):
    ...
    for x in range(current_year, 1954, -1):
        url = host + 'series-latino/año/' + str(x) + '.html'
...
def alfabetico(item):
    ...
    for letra in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789':
        url = host + 'series-latino/alfabeto/' + letra
...
def list_all(item):
    ...
    bloque = scrapertools.find_single_match(data, '<span>Verserie.cc</span>(.*?)>mas vistas<')

    if not bloque: bloque = scrapertools.find_single_match(data, '<span>verserie.cc</span>(.*?)>mas vistas<')

    if not bloque: bloque = scrapertools.find_single_match(data, '<span>verserie</span>(.*?)>mas vistas<')
    if not bloque: bloque = scrapertools.find_single_match(data, '<span>Verserie</span>(.*?)>mas vistas<')
    ...
    for match in matches:
        ...
        year = '-'
        if '/series-latino/año/' in item.url:
            year = scrapertools.find_single_match(item.url, "/series-latino/año/(.*?)$")
            if year:
...
def episodios(item):
    ...
    if item.page == 0 and item.perpage == 50:
        ...
        if config.get_setting('channels_charges', default=True):
            ...
            if sum_parts >= 100:
                platformtools.dialog_notification('VerSerieOnline', '[COLOR cyan]Cargando ' + str(sum_parts) + ' elementos[/COLOR]')
        elif tvdb_id:
            if sum_parts > 50:
                platformtools.dialog_notification('VerSerieOnline', '[COLOR cyan]Cargando Todos los elementos[/COLOR]')
                item.perpage = sum_parts
        else:
            item.perpage = sum_parts
            ...
            if sum_parts >= 1000:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]500[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('VerSerieOnline', '[COLOR cyan]Cargando 500 elementos[/COLOR]')
                    item.perpage = 500

            elif sum_parts >= 500:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]250[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('VerSerieOnline', '[COLOR cyan]Cargando 250 elementos[/COLOR]')
                    item.perpage = 250

            elif sum_parts >= 250:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]125[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('VerSerieOnline', '[COLOR cyan]Cargando 125 elementos[/COLOR]')
                    item.perpage = 125

            elif sum_parts >= 125:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]75[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('VerSerieOnline', '[COLOR cyan]Cargando 75 elementos[/COLOR]')
                    item.perpage = 75
    ...
    for option in options:
        ...
        if d_url:
            ...
            try:
                ...
                if hay_proxies:
                    url = httptools.downloadpage_proxy('VerSerieOnline', d_url, follow_redirects=False, timeout=timeout).headers['location']
                else:
