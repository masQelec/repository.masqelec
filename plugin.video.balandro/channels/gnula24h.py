...
def do_downloadpage(url, post=None, headers=None, raise_weberror=True):
    ...
    if not url.startswith(host):
        ...
        if not data:
            if not '/?s=' in url:
                if config.get_setting('channels_re_charges', default=True): platformtools.dialog_notification('Gnula24H', '[COLOR cyan]Re-Intentando acceso[/COLOR]')
...
def list_all(item):
    ...
    if '>Añadido recientemente<' in data: bloque = scrapertools.find_single_match(data, '>Añadido recientemente<(.*?)>Últimos Emitidos<')
