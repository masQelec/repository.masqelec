...
host = 'https://www2.pelisplushd.lat/'


# ~ por si viene de enlaces guardados
ant_hosts = ['https://pelisplushd.lat/', 'https://www1.pelisplushd.lat/']


domain = config.get_setting('dominio', 'pelisplushdlat', default='')

if domain:
    if domain in str(ant_hosts): config.set_setting('dominio', '', 'pelisplushdlat')
    else: host = domain
    ...
    if item.page == 0:
        ...
        if sum_parts > 250:
            if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]250[/B][/COLOR] elementos?'):
                platformtools.dialog_notification('PelisPlusHdLat', '[COLOR cyan]Cargando elementos[/COLOR]')
                item.perpage = 250
