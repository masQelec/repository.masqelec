...
host = 'https://www.veronline.cloud/'
...
def list_all(item):
    ...
    bloque = scrapertools.find_single_match(data, '<span>Veronline.cloud</span>(.*?)>mas vistas<')

    if not bloque: bloque = scrapertools.find_single_match(data, '<span>veronline.cloud</span>(.*?)>mas vistas<')
