...
def update_search_query(search_id):
	...
	if search_id in list_query_types:
		current_game_lists = IAGL.get_game_lists()
		try:
			current_select = ['Ninguno']+[x for x in current_game_lists.get('emu_name')]
	...
	if search_id == 'genre':
		...
		if current_genre_lists is not None:
			current_genre_lists = ['Ninguno']+current_genre_lists
	...
	if search_id == 'groups':
		...
		if current_groups_lists is not None:
			current_groups_lists = ['Ninguno']+current_groups_lists
	...
	if search_id == 'nplayers':
		...
		if current_players_lists is not None:
			current_players_lists = ['Ninguno']+current_players_lists
	...
	if search_id == 'year':
		...
		if current_year_lists is not None:
			current_year_lists = ['Ninguno']+current_year_lists
	...
	if search_id == 'studio':
		...
		if current_studio_lists is not None:
			current_studio_lists = ['Ninguno']+current_studio_lists
	...
	if current_query['title'] is None:
		...
		if current_query['lists'] is None:
			...
			if len(current_query['lists'])>10:
				...
				create_listitem = True
	default_label = 'IAGL Buscar %(query_value)s' % {'query_value':current_query['title']}
	if create_listitem:
		current_dialog = xbmcgui.Dialog()
...
def generate_random_listitem():
	...
	if current_query['lists'] is None:
		...
		if len(current_query['lists'])>10:
			...
			create_listitem = True
	default_label = 'IAGL Reproducción Aleatoria %(query_value)s' % {'query_value':IAGL.get_random_time()}
	if create_listitem:
		current_dialog = xbmcgui.Dialog()
		new_value = current_dialog.input(IAGL.loc_str(30320),default_label)
		del current_dialog
