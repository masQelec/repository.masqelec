...
if not WIN.getProperty('iagl.script_started'):
	...
	try:
		xbmc.log(msg='IAGL:  Update addon to provide games', level=xbmc.LOGDEBUG)
		...
		try:
			...
			xbmc.log(msg='IAGL Error:  Update addon to provide games, Exception %(exc)s' % {'exc': str(exc1)}, level=xbmc.LOGERROR)
		...
		if False not in success:
			...
			ok_ret = current_dialog.ok('Completed','Addon updated to provide games')
	...
	xbmc.log(msg='IAGL:  Update addon to provide games completed', level=xbmc.LOGDEBUG)
else:
