...
class SRT:
    ...
    def __init__(self, pathOrFile):
        if isinstance(pathOrFile, six.StringIO):
    ...
    def write(self, start, end, text):
        ...
        text = "%0u\n%s --> %s\n%s\n\n" % (self.counter, self._formatTime(start), self._formatTime(end), text)
        self.file.write(text.encode('utf8'))
        self.counter += 1
    ...
    def decode(self, input):
        ...
        for char in input:
            ...
            if not state and char in self.identical:
                ...
                combined = unicodedata.normalize('NFC', six.unichr(char) + six.unichr(state))
                if combined and len(combined) == 1:
                    output.append(ord(combined))
                state = None
        return (''.join(map(six.unichr, output)), len(input))
...
class TT:
    ...
    def _parseXML(self):
        ...
        try:
            xml = ElementTree.fromstring(self.source)
        except:
