# -*- coding: utf-8 -*-
# --------------------------------------------------------------------------------
# WebTV - Updater (actualizaciones del addon)
# --------------------------------------------------------------------------------
...
def check_addon_updates(verbose=False):
    ...
    try:
        ...
        if data == '': 
            ...
            if verbose:
                platformtools.dialog_notification('WebTV ya está actualizado', 'No hay ninguna actualización pendiente')
            return False
        ...
        if os.path.exists(last_fix_json):
            ...
            if lastfix['addon_version'] == data['addon_version'] and lastfix['fix_version'] == data['fix_version']:
                ...
                if verbose:
                    platformtools.dialog_notification('WebTV ya está actualizado', 'Versión %s.fix%d' % (data['addon_version'], data['fix_version']))
                return False
        ...
        if verbose:
            platformtools.dialog_notification('WebTV actualizado a', 'Versión %s.fix%d' % (data['addon_version'], data['fix_version']))
        return True
        ...
        if verbose:
            platformtools.dialog_notification('WebTV actualizaciones', 'Error al comprobar actualizaciones')
        return False
